...
print("🚀 [4주 라이브 스캐너] 유동성 히스토리 추적 및 종목 스캔 시작...")
...
# ==============================================================================
start_history = (datetime.today() - timedelta(days=450)).strftime('%Y-%m-%d')
...
# 4주 라이브 타겟 날짜 설정
target_start_dt = datetime.today() - timedelta(days=35)
...
for i, trade_date in enumerate(all_trading_days):
    ...
    # 🚨 [속도 최적화] 타겟 날짜(최근 4주)가 아니면 무거운 종목 스캔 패스!
...
# ==============================================================================
# 💾 엑셀 작성 및 자동 이메일 전송 세팅
# ==============================================================================
file_name = 'Live_Scanner_4Weeks_Final.xlsx'
...
with pd.ExcelWriter(file_name, engine='xlsxwriter') as writer:
    summary_df.to_excel(writer, sheet_name='📊 4주 시그널 요약', index=False, startrow=1)
    all_stocks_df.to_excel(writer, sheet_name='🎯 4주 포착 종목', index=False, startrow=1)

    wb = writer.book
    ws1 = writer.sheets['📊 4주 시그널 요약']
    ws2 = writer.sheets['🎯 4주 포착 종목']
    ...
    ws1.conditional_format('C3:C1000', {'type': 'text', 'criteria': 'containing', 'value': '🔥', 'format': f_red})
    ws1.conditional_format('C3:C1000', {'type': 'text', 'criteria': 'containing', 'value': '❄️', 'format': f_blue})
    ws1.conditional_format('B3:B1000', {'type': 'formula', 'criteria': 'OR(ISNUMBER(SEARCH("3주",B3)),ISNUMBER(SEARCH("4주",B3)),ISNUMBER(SEARCH("5주",B3)),ISNUMBER(SEARCH("6주",B3)))', 'format': f_mblue})
